...
class ScrumTimer: ObservableObject {
    ...
    @Published var secondsRemaning = 0
    ...
    init(lengthInMinutes: Int = 0, attendees: [DailyScrum.Attendee] = []) {
        ...
        secondsRemaning = lengthInSeconds
    ...
    private func changeToSpeaker(at index: Int) {
        ...
        secondsRemaning = lengthInSeconds - secondsElapsed
        startDate = Date()
    ...
    private func update(secondsElapsed: Int) {
        ...
        secondsRemaning = max(lengthInSeconds - self.secondsElapsed, 0)
        
        guard !timerStopped else { return }
        
        if secondsElapsedForSpeaker >= secondsElapsed {
            changeToSpeaker(at: speakerIndex + 1)
            speakerChangedAction?()
        }
    }
    
    func reset(lengthInMinues: Int, attendees: [DailyScrum.Attendee]) {
        self.lengthInMinutes = lengthInMinues
        self.speakers = attendees.speakers
        secondsRemaning = lengthInSeconds
        activeSpeaker = speakerText
    }
